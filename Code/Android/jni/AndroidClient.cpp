...
int main(int argc, char *argv[])
{
    ...
    LOGD("Stuff happened at %u", __LINE__);

    sf::IpAddress IPADDRESS = sf::IpAddress::getLocalAddress();
    const unsigned short PORT = 6789;
    sf::TcpSocket socket;
    sf::Socket::Status status = socket.connect("192.168.1.70", PORT);

    LOGD("Stuff happened at %u", __LINE__);
    if (status != sf::Socket::Done)
    {
        LOGD("Error in socket");
    ...
    Personnage perso("sprites/SpriteIop01.png");
    ...
    Personnage other("sprites/SpriteIop01.png");
    ...
    if (!textureHealthBar.loadFromFile("bars/healthBar.png"))
    {
    ...
    if (!textureXpBar.loadFromFile("bars/xpBar.png"))
    {
    ...
    if (!texturePA.loadFromFile("icons/IconPA.png"))
    {
    ...
    if (!texturePM.loadFromFile("icons/IconPM.png"))
    {
    ...
    if (!texturePW.loadFromFile("icons/IconPW.png"))
    {
    ...
    for (unsigned int i = 0; i < nbTexturesSortsVisuels; i++)
    {
        ...
        std::string pathToSort = "sprites/Sorts/Sort" + std::to_string(i) + ".png";
        if (!textureSort.loadFromFile(pathToSort))
        {
            std::cout << "Error when loading the texture of Sort" << i << ".png" << std::endl;
            if (!textureSort.loadFromFile("sprites/Sorts/Sort0.png"))
            {
...
//    sortsVisuels.push_back({sf::Vector3f{500.0f, 400.0f, 0.0f}, 2, texturesSortsVisuels, 16u, 7u, 0u, 500u, 2, 1, 128, 0, 1, sf::Time{sf::milliseconds(6000)}});
    ...
    {
        sf::Texture textureSort;
        std::string pathToSort = "icons/IconSorts/IconSort" + std::to_string(i) + ".png";
        if (!textureSort.loadFromFile(pathToSort))
        {
            std::cout << "Error when loading the texture of Sort" << i << ".png" << std::endl;
            if (!textureSort.loadFromFile("icons/IconSorts/IconSort0.png"))
            {
                std::cout << "Le sprite de secours n'a pas pu être chargé non plus..." << std::endl;
                return -1;
            }
    ...
    if (!font.loadFromFile("font/sao.otf"))
    {
